...
- **Auto-Update** -- checks GitHub for new releases on startup, one-step upgrade with progress bar
- **SynthChat** -- Slack-like multi-agent workspace with 6 specialized agents, custom agent creation, community library, document generation, and desktop app mode ([full docs](synthchat/README.md))
...
|---------|-------------|
| `python main.py` | Octobot + SynthChat together at `http://localhost:5000/synthchat/` |
| `python main-chat.py` | SynthChat standalone at `http://localhost:3000` |
| `python desktop.py` | Native desktop window (auto-installs pywebview) |

You can also install SynthChat as a **PWA** -- visit it in Chrome/Edge and click "Install" in the address bar. It gets its own window, taskbar icon, and works like a native app.
...
- **Desktop app** -- run as a native window via `desktop.py` or install as a PWA from the browser
...
Opens a chat interface in your browser at `http://localhost:5000`. Dark theme, markdown rendering, collapsible tool panels, and a swimming octopus loading animation. SynthChat is available at `/synthchat/`.

### SynthChat Desktop App

```bash
python desktop.py
```

Opens SynthChat as a native desktop window. Starts the server in the background, opens the app, and shuts everything down when you close the window. Auto-installs `pywebview` on first run.
...
```bash
python main.py --skip-update
```

Bypasses the automatic GitHub release check on startup.
...
main-chat.py             SynthChat standalone (port 3000)
desktop.py               SynthChat desktop app (pywebview native window)
start.sh / start.bat     Auto-install launcher scripts
VERSION                  Current version (used by auto-updater)
octobot/
  ...
  updater.py             GitHub release checker + auto-updater with progress bar
...
synthchat/               Multi-agent workspace (see synthchat/README.md)
community/agents/        Pre-made agent catalog (5 agents)
